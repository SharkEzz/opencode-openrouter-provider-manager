...
2. Add the plugin package to `~/.config/opencode/opencode.jsonc`, replacing the example path with the absolute path of your checkout:

   ```jsonc
   {
     "$schema": "https://opencode.ai/config.json",
     "plugins": ["/absolute/path/to/openrouter-provider-manager/packages/plugin"],
   }
   ```

   If the file already has a `plugins` array, add the path to that array. Point it at `packages/plugin`, not at the repository root: the root is a pnpm workspace, not a plugin. OpenCode loads the package's server and TUI entry points together; no separate `cli.json` entry is needed. Keep the checkout outside `~/.config/opencode/plugins/`: that directory holds plugins, and the repository root is not one.
...
From the repository root, run `pnpm typecheck`, `pnpm test`, `pnpm format:check`, and `pnpm lint:check` before submitting changes. Vitest covers the routing and endpoint logic; verify picker changes in OpenCode as well. See [AGENTS.md](../../AGENTS.md) for repository conventions.
