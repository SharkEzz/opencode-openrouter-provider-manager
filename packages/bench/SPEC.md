...
- Le summary (`results/summary.json`, sinon `results/summary.fake.json`) est validé au build par `schema.ts` (un résultat invalide fait échouer le build), puis servi comme fichier statique `summary.json` et chargé à l'exécution avec `use()` + `<Suspense>` : les données restent hors du bundle JS. `pnpm build` refuse un summary synthétique ; `pnpm build:preview` l'accepte, avec un bandeau permanent et `noindex`.
- Polices auto-hébergées (`@fontsource-variable/inter`, `@fontsource-variable/jetbrains-mono`) : pas d'appel à Google Fonts ni à un CDN.
