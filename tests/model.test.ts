...
describe("resolveModel", () => {
  it("uses the session's model when nothing was picked on this screen", () => {
    const lastPicked = vi.fn(() => luna)
    expect(resolveModel({ sessionID: "ses_1", sessionModel: sol, pick: undefined, lastPicked })).toBe(sol)
  ...
  it("prefers a pick made on this screen over the session's model", () => {
    expect(resolveModel({ sessionID: "ses_1", sessionModel: sol, pick: luna, lastPicked: () => undefined })).toBe(luna)
  })

  it("falls back to the last pick on the home screen", () => {
    expect(resolveModel({ sessionID: undefined, sessionModel: undefined, pick: undefined, lastPicked: () => luna })).toBe(
      luna,
    )
  })

  it("prefers the home screen's own pick", () => {
    expect(resolveModel({ sessionID: undefined, sessionModel: undefined, pick: sol, lastPicked: () => luna })).toBe(sol)
  })

  it("returns nothing when no model is known", () => {
    expect(
      resolveModel({ sessionID: "ses_1", sessionModel: undefined, pick: undefined, lastPicked: () => luna }),
    ).toBeUndefined()
  })
})
